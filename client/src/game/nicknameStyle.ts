...
// Pulse·네온사인·글리치는 그 자체가 이미 애니메이션되는 text-shadow라, 독립 글로우의
// 인라인 style.textShadow를 얹으면 인라인 스타일이 CSS 클래스의 애니메이션 그림자를
// 덮어써서 깜빡임/숨쉬기/색수차 자체가 죽는다(단순히 안 예쁜 수준이 아니라 실제로 효과가
// 사라지는 버그) — 그래서 이 셋일 땐 글로우를 아예 계산하지 않는다.
const NO_INDEPENDENT_GLOW = new Set<NicknameEffect>(["pulse", "neon", "glitch"]);

const PARTICLE_DOT_COUNT = 10;
// 별똥별은 한 줄(14px 대각선 트레일)이 점(3~4px)보다 훨씬 존재감이 커서, 다른 파티클과
// 같은 10개를 그대로 쓰면 화면이 과하게 붐빈다 — 6개로 줄임.
const SHOOTING_STAR_DOT_COUNT = 6;
const PARTICLE_SIMPLE_CLASSNAME: Record<Exclude<NicknameParticle, "none" | "orbit">, string> = {
  ...
  snow: styles.snowDot,
  heart: styles.heartDot,
  shootingStar: styles.shootingStarDot,
};
...
// 점 하나하나(기본 PARTICLE_DOT_COUNT개, 별똥별만 SHOOTING_STAR_DOT_COUNT개)가 실제 DOM
// 엘리먼트라 닉네임 폭 전체(4%~96%)에서 각자 독립적인 위치·타이밍으로 스폰된다 — 색상
// 문자열 시드 기반이라 사실상 랜덤처럼 보이지만 같은 사람은 항상 같은 배치를 유지한다.
function particleDots(
  ...
  const duration = particle === "snow" ? 2.8 : particle === "shootingStar" ? 4.2 : 2.6;
  const dotCount = particle === "shootingStar" ? SHOOTING_STAR_DOT_COUNT : PARTICLE_DOT_COUNT;
  ...
  for (let i = 0; i < dotCount; i++) {
    ...
    if (particle === "twinkle" || particle === "shootingStar") {
      // 반짝임/별똥별은 이동 방향이 이미 CSS 키프레임에 고정돼 있어(반짝임은 이동 없음,
      // 별똥별은 정해진 대각선) 눈/상승/하트처럼 한쪽 가장자리에서 드리프트할 필요가 없다
      // — 세로 위치도 폭 전체에 걸쳐 자유롭게 흩뿌리기만 하면 된다.
      const topPct = 4 + seededUnit(seed, 300 + i * 13) * 92;
      style.top = `${topPct.toFixed(1)}%`;
    } else {
      // 상승/눈/하트는 낙하·상승 중 좌우로 흔들리는 drift(바람에 날리는 느낌) —
...
// Pulse/네온사인/크롬/글리치는 서로 배타적(닉네임의 "기본 색"을 정의하는 효과라 동시에
// 켤 수 없음 — nicknameEffect가 이미 하나의 값만 가지므로 구조적으로 보장됨). 글로우와
