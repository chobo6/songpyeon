...
const PARTICLE_SIMPLE_CLASSNAME: Record<Exclude<NicknameParticle, "none">, string> = {
  twinkle: styles.twinkleDot,
  rising: styles.risingDot,
  snow: styles.snowDot,
  heart: styles.heartDot,
  petal: styles.petalDot,
};
...
// 점 하나하나(PARTICLE_DOT_COUNT개)가 실제 DOM 엘리먼트라 닉네임 폭 전체(4%~96%)에서
// 각자 독립적인 위치·타이밍으로 스폰된다 — 색상 문자열 시드 기반이라 사실상 랜덤처럼
// 보이지만 같은 사람은 항상 같은 배치를 유지한다.
function particleDots(
  particle: Exclude<NicknameParticle, "none">,
  color: string | null | undefined,
): ParticleDot[] {
  const seed = stringSeed(color || "particle-default-seed");
  const dotClass = PARTICLE_SIMPLE_CLASSNAME[particle];
  const duration = particle === "snow" ? 2.8 : particle === "petal" ? 3.2 : 2.6;
  ...
  for (let i = 0; i < PARTICLE_DOT_COUNT; i++) {
    ...
    if (particle === "twinkle") {
      // 반짝임은 이동 방향이 이미 CSS 키프레임에 고정돼 있어(이동 없이 제자리에서 펄스)
      // 눈/상승/하트/벚꽃처럼 한쪽 가장자리에서 드리프트할 필요가 없다 — 세로 위치도
      // 폭 전체에 걸쳐 자유롭게 흩뿌리기만 하면 된다.
      const topPct = 4 + seededUnit(seed, 300 + i * 13) * 92;
      style.top = `${topPct.toFixed(1)}%`;
    } else {
      // 상승/눈/하트/벚꽃은 낙하·상승 중 좌우로 흔들리는 drift(바람에 날리는 느낌) —
...
// Pulse/네온사인/크롬/글리치/서리는 서로 배타적(닉네임의 "기본 색"을 정의하는 효과라 동시에
