...
## 향후 방향 (이번 범위 밖, 참고용 메모)

사용자 확인(2026-07-25): 시간추가/시간감소/반죽공격/슈퍼절구 4개 아이템도 궁극적으로는 이 절구회복 토큰과 같은 방식(시퀀스 안에 확률적으로 딸려나오는 토큰)으로 획득하게 될 예정. 다만 그 4개는 절구회복과 달리 "보유" 단계가 있음 — 각 플레이어(역할)의 버튼패널 6칸 중 안 쓰는 2칸을 그 플레이어 개인 인벤토리로 써서, 토큰 성공 시 그 자리(누른 사람의 인벤토리)에 아이템이 담긴다. 이미 2개 차 있으면 새로 획득한 아이템은 그냥 소멸(교체 없음). 이렇게 되면 기존 `useItem`의 권한 체크("우리 팀 소속이면 아무나")도 "그 아이템을 실제로 보유한 그 플레이어만"으로 바뀌어야 함 — 이미 배포된 4개 아이템 시스템에 대한 리트로핏이 필요한 별도 프로젝트로, 이번 절구회복 토큰 작업과는 분리해서 나중에 따로 브레인스토밍하기로 함. 절구회복은 이 인벤토리 개념이 아예 필요 없음(예외적으로 즉시 발동) — 그래서 이번 스펙/계획은 그대로 유효.
