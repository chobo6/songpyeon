...
## #29 온라인 매치 돼지 키보드 테스트 매핑 — 임시, 미커밋 상태로 배포됨 (제거 방법 기록)

### 배경

키보드/매크로 의심 유저가 늘어 관리자 페이지에 실시간 모니터링(`AdminPressMonitor.tsx`,
`#26` 이전 항목들 참고)과 돼지 4색 스팸 가드(`PIG_SPAM_THRESHOLD_MS`, `inputSpamGuard.ts`)를
붙였는데, 실제 온라인 매치에서 마우스/터치 클릭이 아니라 진짜 키보드 입력으로 눌렀을 때
가드가 체감상 어떻게 동작하는지 직접 테스트해볼 필요가 생김. `AdminKeyTestTool.tsx`(관리자
페이지 전용, 서버 게임 로직과 무관한 독립 화면)와 달리 이건 **실제 온라인 매치 화면
(`MyTurnScreen.tsx`)에 직접 심은 임시 키보드 매핑**이라 별도로 기록.

### 내용

`client/src/components/MyTurnScreen.tsx`에 `useEffect` 블록 하나만 추가 — 돼지 역할이고 내
턴이 활성 상태(`!disabled`)일 때, s/a/d/m 키를 각각 red/yellow/orange/purple에 매핑해 기존
`press()` 콜백(서버로 `pressButton` 메시지 전송, `ButtonPanel`의 클릭 핸들러와 동일한
경로)을 그대로 호출한다. `e.repeat`으로 키를 누른 채 있을 때의 OS 자동 반복은 무시.

```tsx
// TEMP: 돼지 키보드 테스트용 임시 매핑 — 검증 끝나면 이 useEffect 블록 통째로 삭제.
useEffect(() => {
  if (me.role !== "pig" || disabled) return;
  const KEY_TO_COLOR: Record<string, Color> = { s: "red", a: "yellow", d: "orange", m: "purple" };
  function onKeyDown(e: KeyboardEvent) {
    if (e.repeat) return;
  ...
  window.addEventListener("keydown", onKeyDown);
  return () => window.removeEventListener("keydown", onKeyDown);
}, [me.role, disabled, press]);
```

`import { useCallback, useEffect } from "react";`의 `useEffect`도 이 블록과 함께 추가됨.

### 제거 방법

위 `useEffect` 블록 전체와, 다른 곳에서 `useEffect`를 안 쓴다면 import의 `useEffect`도 같이
삭제. 그 외 파일은 건드리지 않았음(`ButtonPanel.tsx`, 서버 쪽 전부 그대로).

### 상태

~~git에 커밋되지 않은 상태로 프로덕션에 배포됨~~ → **검증 끝나고 코드 제거, 재배포 완료
(같은 날)**. 위 코드는 실제로는 더 이상 존재하지 않음 — 나중에 비슷한 임시 도구가 또 필요할
때 그대로 복사해서 쓰라고 기록만 남겨둠(`#25`의 계측 도구 기록과 같은 용도).

### 관련 파일
- `client/src/components/MyTurnScreen.tsx`

## #30 온라인 매치 돼지/토끼 입력 연타 속도 제한 임시 해제 — 미커밋 상태로 배포됨 (제거 방법 기록)

### 배경

`#29`의 돼지 키보드 매핑에 이어, 이번엔 `inputSpamGuard`(민트 35ms / 돼지 4색 5ms) 가드 자체를
잠깐 꺼두고 테스트해볼 필요가 생김. `isSpammedPress` 함수나 그 테스트(`inputSpamGuard.test.ts`)는
그대로 두고, **실제로 입력을 막는 지점만** 임시 스위치로 우회.

### 내용

`server/src/rooms/MatchRoom.ts` 상단에 플래그 하나 추가:

```ts
// TEMP: 연타 속도 제한(inputSpamGuard) 실제 차단 여부 — 검증 끝나면 true로 되돌릴
// 것. false로 두 상태로 배포됨(docs/TROUBLESHOOTING.md #29 참고).
const SPAM_GUARD_ENFORCED = false;
```

`handlePressButton`의 차단 분기를 이 플래그로 감쌈(원래는 `if (blocked)`이었음):

```ts
// TEMP: 연타 속도 제한 임시 해제 — 검증 끝나면 SPAM_GUARD_ENFORCED를 true로
// 되돌릴 것. isSpammedPress/blocked 계산은 그대로 둬서 관리자 모니터링
// 화면에는 계속 blocked 여부가 표시됨 — 실제로 입력을 막지만 않을 뿐.
if (SPAM_GUARD_ENFORCED && blocked) {
...
`isSpammedPress` 호출과 `blocked` 계산, 관리자 모니터링(`notifyPress`)으로의 보고는 그대로
살아있음 — 그래서 이 상태에서도 `AdminPressMonitor.tsx`에는 여전히 `blocked: true`가 찍히는
입력이 보이지만, 실제로 게임에 반영되지는 않고(절구 감점/오답 판정 없이) 그대로 통과됨.

### 제거 방법

`SPAM_GUARD_ENFORCED`를 `true`로 바꾸거나(가장 간단), 아니면 `#29`처럼 플래그와 관련 주석을
통째로 지우고 원래의 `if (blocked)`로 되돌려도 됨. 서버 쪽 변경만 있고 클라이언트는 무관.

### 상태

~~git에 커밋되지 않은 상태로 프로덕션에 배포됨~~ → **검증 끝나고 플래그/분기 제거, 재배포
완료(같은 날)**. `npm run build`(tsc) + `npm test`(185/185) 모두 통과 확인 후 배포했었고,
원복 후에도 다시 빌드+테스트 통과 확인함. 위 코드는 실제로는 더 이상 존재하지 않음 — 기록만
남겨둠.

### 관련 파일
- `server/src/rooms/MatchRoom.ts`

## #31 로컬 `npm run sync-public` 잔재가 배포 이미지에 그대로 딸려 들어감 — `.dockerignore` 누락

### 증상

`#29`/`#30` 임시 기능을 원복하고 재배포하려고 이미지를 빌드했는데, 컨테이너 안
`/app/server/public/assets/`에 최신 번들(`index-Y9DI9rKG.js`) 말고도 **원복 전 로컬 테스트용
빌드**(`index-B3-IucDI.js`, `#29`의 돼지 키보드 매핑 코드가 그대로 든 파일)가 같이 들어있는
걸 발견함. 서비스되는 `index.html`은 항상 최신 해시만 가리켜서 기능적으로 보이는 문제는
없었지만, 임시/디버그 코드가 든 옛 파일이 프로덕션 컨테이너 파일시스템에 물리적으로 남아있게
되는 셈이라 "원복했다"는 말이 사실과 어긋나게 됨.

### 원인

`Dockerfile`은 `COPY server/ server/`(서버 소스 전체, 여기엔 로컬 `npm run sync-public`이
써둔 `server/public/`도 포함됨)를 실행한 뒤, `COPY --from=client-build /app/client/dist
./server/public`로 방금 컨테이너 안에서 새로 빌드한 결과물을 그 위에 다시 복사한다. Docker의
`COPY dst`가 기존 디렉토리를 **삭제 후 교체가 아니라 병합**하기 때문에, 두 번째 COPY에 없는
파일(로컬 sync-public이 남긴 옛 해시 번들)은 지워지지 않고 그대로 남는다. `.dockerignore`에
`client/dist`는 있었지만 `server/public`은 빠져 있어서, 애초에 로컬 잔재가 빌드 컨텍스트에
포함되는 것부터 막지 못했음.

### 수정

`.dockerignore`에 `server/public` 추가 — 로컬 dev 전용 산출물(`npm run sync-public`이 매번
새로 만드는 것)이라 이미지 빌드 컨텍스트에 애초에 들어갈 필요가 없음. 컨테이너 안의
`server/public`은 항상 `client-build` 스테이지가 그 자리에서 새로 만든 것만 있어야 함.

### 검증

수정 후 이미지를 다시 빌드해 `docker run --rm songpyeon:latest sh -c "ls
/app/server/public/assets/"`로 최신 해시 파일 하나(js+css)만 있는지 확인. 배포 후에도 헬스체크
(`/`, `/api/rooms`, `/admin`) 정상 통과.

### 교훈

로컬에서 `sync-public`을 자주 돌리는 습관(관리자 페이지/구글 로그인처럼 same-origin 필요한
기능을 2567에서 확인할 때 — CLAUDE.md 참고)과 실제 배포 이미지 빌드가 같은 `server/public`
디렉토리를 공유하고 있었던 게 근본 원인. 앞으로 로컬 산출물과 이미지 빌드 산출물이 같은
경로를 쓰는 다른 곳이 또 생기면 `.dockerignore`부터 확인할 것.

### 관련 파일
- `.dockerignore`, `Dockerfile`, `sync-public.js`
